...
export async function generateMetadata({params}:{params:Promise<{category:string}>}):Promise<Metadata>{
  ...
  return{title:cat.label,description:`${cat.description} Free, fast and private online tools from Toolmera.`,alternates:{canonical:`https://toolmera.com/${category}/`}}
}

export default async function CategoryPage({params}:{params:Promise<{category:string}>}){
  const {category}=await params;const cat=categories.find(c=>c.slug===category);if(!cat)notFound();const list=toolsForCategory(category);
  return <><Header/><main className="subPage">
    <section className="shell categoryHero compactHero">
      ...
      <h1>{cat.label}</h1>
      <p>{cat.description} Fast, focused and designed with privacy in mind.</p>
    ...
    <section className="shell seoPanel categorySeoPanel">
